...
public class ScrapService {
    ...
    @Transactional
    public ScrapResponse.DTO save(Integer id, ScrapRequest.SaveDTO reqDTO, SessionUser sessionUser) {
        User user = userJPARepository.findById(sessionUser.getId())
                .orElseThrow(() -> new Exception404("존재 하지 않는 계정입니다"));
        Notice notice = noticeJPARepository.findById(id)
                .orElseThrow(() -> new Exception404("존재 하지 않는 공고입니다"));
        Resume resume = resumeJPARepository.findById(id)
                .orElseThrow(() -> new Exception404("존재 하지 않는 이력서입니다"));
